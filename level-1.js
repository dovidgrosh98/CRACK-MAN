...
function board() {
    ...
    setInterval( function() {
        function ghostMovement(x, y) {
            ghostEl = document.querySelector('.ghosts')
        ...
        ghostMovement(ghosts[0].x, ghosts[0].y)
    }, 300)


    function moveCharacterTo(x, y) {
        const capmanChar = document.querySelector('#character');
        capmanChar.style.top = `${(y * 30).toString()}px`;
        capmanChar.style.left = `${(x * 30).toString()}px`;
        if (ballInSpot(x, y)) {
            removeBall(x, y);
            ballPosition();
            if (balls <= 0) {
